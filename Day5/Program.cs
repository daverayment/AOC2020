...
namespace Day5
{
    class Program
    {
        ...
        {
            int maxSeatId = 0;
            var assignedSeatIds = new List<int>();

            foreach (string pass in Passes)
            {
                (int row, int column) = Decode(pass);
                int seatId = row * 8 + column;
                assignedSeatIds.Add(seatId);
                maxSeatId = Math.Max(maxSeatId, seatId);
            }
            Console.WriteLine($"Q1: Max SeatID: {maxSeatId}.");
            ...
            for (int i = 0; i < maxSeatId; i++)
            {
                if (!assignedSeatIds.Contains(i) &&
                    ...
                    assignedSeatIds.Contains(i - 8))
                {
                    Console.WriteLine($"Q2: Seat ID: {i}, Row {i / 8}, Column {i % 8}.");
                    break;
                }
            }
        }

        static (int, int) Decode(string pass)
        {
            int range = 128;
            int row = 128;
            int col = 8;
...
FBBFFBFLLL
BBFFBFFRLL
FBFFFBFLLL".Split(Environment.NewLine);
    }
}