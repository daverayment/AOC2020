...
namespace Day4
{
    class Program
    {
        ...
        static List<string> ValidEyeColours = new List<string> {
                "amb", "blu", "brn", "gry", "grn", "hzl", "oth" };

        static void Main(string[] args)
        {
            (int total, int valid) = Validate();
            Console.WriteLine($"Q1: total passports: {total}. Valid: {valid}.");
            (total, valid) = Validate(strict: true);
            Console.WriteLine($"Q2: total passports: {total}. Valid: {valid}.");
        }

        static (int, int) Validate(bool strict = false)
        {
            var fieldsPresent = new List<string>(new string[] { "cid" });
            ...
            for (int i = 0; i < Input.Length; i++)
            {
                ...
                foreach (string pair in nvPairs)
                {
                    ...
                    if (ValidFields.Contains(nv[0]) &&
                        !fieldsPresent.Contains(nv[0]))
                    {
                        if (strict)
                        {
                            bool valueOk = false;
...
hgt:183cm cid:187 byr:2019 ecl:xry iyr:2013 pid:164cm hcl:#18171d eyr:2021".Split(new string[] { "\r\n" }, StringSplitOptions.None);

// static string[] Input = @"ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
...
// iyr:2011 ecl:brn hgt:59in".Split(new string[] { Environment.NewLine }, StringSplitOptions.None);
    }
